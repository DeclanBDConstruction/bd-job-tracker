...
// Order/labels must stay in sync with TRAINING_COMPETENCY_KEYS in ramsGenerator.js and db.js
// (kept as a separate literal here, like the rest of this file's duplicated per-renderer
// constants, rather than a shared import, to avoid a require cycle with db.js).
const TRAINING_COMPETENCY_ROWS = [
  ['useOfLadders', 'Use of Ladders'],
  ['scaffoldTowers', 'Scaffold Towers'],
  ['mobileAccessPlatforms', 'Mobile Access Platforms'],
  ['harnesses', 'Harnesses'],
  ['asbestosAwareness', 'Asbestos Awareness'],
  ['signErection', 'Sign Erection'],
  ['excavations', 'Excavations'],
  ['electricalDisconnectionInstallation', 'Electrical Disconnection/Installation'],
];

const RAMS_DISCLAIMER = 'All work will be undertaken by qualified competent persons with experience of the type of work described above and in all cases in full accordance with safety procedures specified in the company’s Health and Safety Policy.';

// Renders a self-contained, printable HTML document for an office-authored "Create RAMS"
// record (see ramsGenerator.js + db.js generated_rams) - a full Method Statement, laid out to
// match BD Construction's own Method Statement paperwork, with the hazard risk-assessment
// table(s) underneath, in the same visual style as renderHtml/renderRamsHtml above. Unlike
// renderRamsHtml (one operative's per-assignment submission, signed on site), this is drafted
// by the office before work starts and has no signature - it's reviewed/adjusted and then
// attached to the job like any other risk assessment (see the
// /api/jobs/:id/risk-assessments/generated/:raId/attach route in server.js).
function renderGeneratedRamsHtml(rams) {
  const list = (items) => `<ul>${(items || []).map((i) => `<li>${escapeHtml(i)}</li>`).join('')}</ul>`;
  const preparedDate = new Date(rams.createdAt).toLocaleDateString('en-GB');
  const keyRows = RISK_KEY_ROWS.map(([l, c, r, slug], i) => `
    <tr>
      ${i === 0 ? `<td rowspan="${RISK_KEY_ROWS.length}" class="company-cell"><span class="company-name">${escapeHtml(COMPANY_NAME)}</span><br><span class="company-address">${escapeHtml(COMPANY_ADDRESS)}</span></td>` : ''}
      <td>${escapeHtml(l)}</td><td>${escapeHtml(c)}</td><td class="key-band ${slug}">${escapeHtml(r)}</td>
  ...
  const operativeRows = [
    rams.supervisorName ? `<tr><td class="section-title" style="width:20%">Supervisor</td><td colspan="3">${escapeHtml(rams.supervisorName)}</td></tr>` : '',
    ...(rams.employees || [])
      .filter((name) => name !== rams.supervisorName)
      .map((name) => `<tr><td class="section-title">Operative</td><td colspan="3">${escapeHtml(name)}</td></tr>`),
    rams.contractorDriver ? `<tr><td class="section-title">Contractor/Driver</td><td colspan="3">${escapeHtml(rams.contractorDriver)}</td></tr>` : '',
  ].join('');

  const trainingRows = TRAINING_COMPETENCY_ROWS.map(([key, label]) => `
    <tr><td>${escapeHtml(label)}</td><td class="lc-cell" style="width:15%">${rams.trainingCompetencies && rams.trainingCompetencies[key] ? 'Yes' : 'No'}</td></tr>
  `).join('');

  const sequenceRows = (rams.sequenceOfOperations || []).length
    ? `<ol>${rams.sequenceOfOperations.map((step) => `<li>${escapeHtml(step)}</li>`).join('')}</ol>`
    : '<p>&nbsp;</p>';

  const hazardBlocks = (rams.hazards || []).map((h) => `
    <table style="margin-top:14px">
      <tr>
        <th class="hazard-table" style="width:20%">Hazard &amp; Potential Harm</th>
        <th class="hazard-table" style="width:28%">Current Risk Controls</th>
      ...
      <tr>
        ...
        <td class="lc-cell">${h.currentL}</td>
        <td class="lc-cell">${h.currentC}</td>
        <td class="r-cell key-band ${h.currentBand.slug}">${h.currentR}</td>
        <td>${list(h.additionalControls)}</td>
        <td class="lc-cell">${h.additionalL}</td>
        <td class="lc-cell">${h.additionalC}</td>
        <td class="r-cell key-band ${h.additionalBand.slug}">${h.additionalR}</td>
      </tr>
      ${(h.ppe && h.ppe.length) ? `<tr><td class="section-title">PPE Required</td><td colspan="8">${escapeHtml(h.ppe.join(', '))}</td></tr>` : ''}
    </table>
  `).join('');
...
<head>
<meta charset="UTF-8">
<title>RAMS — ${escapeHtml(rams.client)} — ${escapeHtml(rams.projectReference)}</title>
<style>
  body { font-family: Arial, Helvetica, sans-serif; color: #1e2733; max-width: 900px; margin: 24px auto; padding: 0 20px; line-height: 1.4; font-size: 13px; }
  table { border-collapse: collapse; width: 100%; }
  th, td { border: 1px solid #999; padding: 6px 8px; text-align: left; vertical-align: top; }
...
<body>
  ...
  <table style="margin-top:14px">
    <tr><td class="section-title" style="width:20%">Client</td><td>${escapeHtml(rams.client)}</td><td class="section-title" style="width:20%">Fitting Date</td><td>${escapeHtml(rams.startDate)}</td></tr>
    <tr><td class="section-title">Project Reference</td><td>${escapeHtml(rams.projectReference)}</td><td class="section-title">Job Number</td><td>${escapeHtml(rams.jobNumber || '—')}</td></tr>
    <tr><td class="section-title">Location/Address</td><td colspan="3">${escapeHtml(rams.location)}</td></tr>
    <tr><td class="section-title">Site Contact</td><td>${escapeHtml(rams.siteContact || '—')}</td><td class="section-title">Tel</td><td>${escapeHtml(rams.siteContactTel || '—')}</td></tr>
  </table>

  <table style="margin-top:14px">
    <tr><td class="section-title" style="width:20%">Description/Summary of Work</td><td colspan="3">${escapeHtml(rams.descriptionOfWork)}</td></tr>
  </table>

  <table style="margin-top:14px">
    ${operativeRows || '<tr><td class="section-title" style="width:20%">Operatives</td><td colspan="3">—</td></tr>'}
  </table>

  <table style="margin-top:14px">
    <tr><td class="section-title" style="width:20%">Access Equipment Description</td><td colspan="3">${escapeHtml(rams.accessEquipmentDescription || '—')}</td></tr>
    <tr><td class="section-title">Access Requirements / Weight Loading / Max Height</td><td colspan="3">${escapeHtml(rams.accessRequirements || 'N/A')}</td></tr>
    <tr><td class="section-title">Other Plant or Tools to be Used</td><td colspan="3">${escapeHtml(rams.otherPlantOrTools || '—')}</td></tr>
    <tr><td class="section-title">Summary of Key Identified Hazards</td><td colspan="3">${escapeHtml(rams.keyHazardsSummary || '—')}</td></tr>
  </table>

  <table style="margin-top:14px">
    <tr><th colspan="2" class="hazard-table">Training Competencies of Team Members</th></tr>
    ${trainingRows}
  </table>

  <table style="margin-top:14px">
    <tr><td class="section-title" style="width:20%">Required Personal Protective Equipment</td><td colspan="3">${escapeHtml((rams.requiredPpe || []).join(', ') || '—')}</td></tr>
    <tr><td class="section-title">Fall Protection Measures &amp; Emergency Procedure</td><td colspan="3">${escapeHtml(rams.fallProtectionMeasures || 'N/A')}</td></tr>
    <tr><td class="section-title">Work Area Protection Arrangements</td><td colspan="3">${escapeHtml(rams.workAreaProtection || '—')}</td></tr>
    <tr><td class="section-title">Location of 1st Aid Box</td><td>${escapeHtml(rams.firstAidLocation || '—')}</td><td class="section-title">Nearest A&amp;E</td><td>${escapeHtml(rams.nearestAE || '—')}</td></tr>
  </table>

  <table style="margin-top:14px">
    <tr><td class="section-title" style="width:100%">Sequence of Operations – Step-by-Step Guide</td></tr>
    <tr><td>${sequenceRows}</td></tr>
  </table>

  ${hazardBlocks}

  <div class="notice">This RAMS was drafted with AI assistance from the information entered and must be reviewed and adjusted for the specific site and task conditions before anyone works to it.</div>

  <table style="margin-top:14px">
    <tr><td class="section-title" style="width:20%">Disclaimer</td><td colspan="3">${escapeHtml(RAMS_DISCLAIMER)}</td></tr>
  </table>

  <table class="footer-table" style="margin-top:20px">
    <tr><td>Prepared By</td><td class="value">${escapeHtml(rams.createdBy || '')}</td></tr>
    <tr><td>Date</td><td class="value">${preparedDate}</td></tr>
    <tr><td>Approved By</td><td class="value">&nbsp;</td></tr>
    <tr><td>Date</td><td class="value">&nbsp;</td></tr>
  </table>
</body>
</html>
`;
...
module.exports = { listRiskAssessments, getRiskAssessment, renderHtml, renderRamsHtml, renderGeneratedRamsHtml, riskBand };
